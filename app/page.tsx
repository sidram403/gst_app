...
import { Menu, Home, FileText, MessageSquare, Calculator, Settings, LogOut, DollarSign, TrendingUp, AlertCircle, Sun, Moon, CheckCircle, Cpu, PieChart, BarChart, FileSpreadsheet, BookOpen } from 'lucide-react'
...
type BillingScreenProps = {
  onSave: (type: 'sales' | 'purchase', data: Omit<Transaction, 'id' | 'type'>) => void,
};
...
const InvoiceItem = ({ transaction, onView } : InvoiceItemProps) => (
  <div className="flex justify-between items-center py-2 border-b last:border-b-0">
    ...
    <div className="flex items-center">
      <span className="text-sm font-semibold mr-2">₹{transaction.amount.toFixed(2)}</span>
...
const DashboardContent = ({ transactions, onViewInvoice } :any ) => {
  ...
  const gstMessages = [
    "GST filing deadline: 20th September 2023 (Quarterly)",
    "Annual GST filing due: 31st December 2023",
    "Next GST payment: 15th October 2023",
    "Quarterly GST audit review: 10th November 2023",
    "Upcoming GST rate revision: 1st January 2024",
  ];
  ...
  const totalBilling = transactions.reduce((sum : number, t :any) => sum + t.amount, 0)
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">+{transactions.length} new</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
      ...
      <div className="grid grid-cols-4 gap-4">
        {microservices.map((service, index) => (
          <MicroserviceIcon key={index} icon={service.icon} title={service.title} />
        ))}
      </div>
      <Card>
        <CardHeader>
        ...
        <CardContent>
          <ScrollArea className="h-[250px]">
            {transactions.slice(0, 10).map((transaction:any) => (
              <InvoiceItem key={transaction.id} transaction={transaction} onView={onViewInvoice} />
            ))}
          </ScrollArea>
        </CardContent>
      </Card>
    ...
    <div className="space-y-4">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="amount" className="text-sm">Amount (before GST)</Label>
            <Input id="amount" name="amount" type="number" step="0.01" required className="text-sm" />
          </div>
          <div className="space-y-2">
            <Label htmlFor="cgst" className="text-sm">CGST (%)</Label>
            <Input id="cgst" name="cgst" type="number" step="0.01" required className="text-sm" />
          </div>
          <div className="space-y-2">
            <Label htmlFor="sgst" className="text-sm">SGST (%)</Label>
            <Input id="sgst" name="sgst" type="number" step="0.01" required className="text-sm" />
          </div>
          <div className="space-y-2">
            <Label htmlFor="igst" className="text-sm">IGST (%)</Label>
            <Input id="igst" name="igst" type="number" step="0.01" required className="text-sm" />
          </div>
        </div>
        <div className="space-y-2">
  ...
  <div className="space-y-4">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      <Card>
        ...
        <CardContent className="text-xs sm:text-sm">XYZ Enterprises Pvt Ltd</CardContent>
      </Card>
      <Card>
        <CardHeader>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-sm sm:text-base">Registration Date</CardTitle>
        </CardHeader>
      ...
      <Card>
        ...
        <CardContent className="text-xs sm:text-sm">Private Limited Company</CardContent>
      </Card>
    </div>
    <Card>
      <CardHeader>
...
export default function EnhancedFinancialApp() {
  ...
  const handleSaveEntry = (type: 'sales' | 'purchase', data: Omit<Transaction, 'id' | 'type'>) => {
  ...
  const renderContent = () => {
    switch (activeItem) {
      case 'Dashboard':
        return <DashboardContent transactions={transactions} onViewInvoice={handleViewInvoice} />
      case 'Billing':
  ...
  return (
    <div className={`min-h-screen bg-background text-foreground ${darkMode ? 'dark' : ''}`}>
      <div className="flex h-screen overflow-hidden">
        {/* Sidebar */}
        <aside className={`w-64 bg-background border-r transition-all duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 fixed md:static top-0 left-0 bottom-0 z-50`}>
          <div className="p-4">
            <h1 className="text-xl md:text-2xl font-bold mb-6">FinApp</h1>
      ...
      <Dialog open={invoiceDialogOpen} onOpenChange={setInvoiceDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          ...
          <div className="py-4">
            {selectedInvoice && (
              <div className="space-y-2">
                <p className="text-sm"><strong>Invoice ID:</strong> {selectedInvoice.id}</p>
                <p className="text-sm"><strong>Type:</strong> {selectedInvoice.type}</p>
                <p className="text-sm"><strong>Date:</strong> {selectedInvoice.date}</p>
                <p className="text-sm"><strong>Party Name:</strong> {selectedInvoice.partyName}</p>
                <p className="text-sm"><strong>Amount:</strong> ₹{selectedInvoice.amount.toFixed(2)}</p>
                <p className="text-sm"><strong>CGST:</strong> {selectedInvoice.gst.cgst}%</p>
                <p className="text-sm"><strong>SGST:</strong> {selectedInvoice.gst.sgst}%</p>
                <p className="text-sm"><strong>IGST:</strong> {selectedInvoice.gst.igst}%</p>
                <p className="text-sm"><strong>Description:</strong> {selectedInvoice.description}</p>
              </div>
            )}
          </div>
        </DialogContent>
